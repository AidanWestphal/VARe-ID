...
def extrapolate_ggr_gps(imgtable, geometry, doctest_mode=False):
    ...
    Example:
        ...
        >>> table = ImageTable("doctest_files/test_dataset", ["grevy's zebra"])
    ...
    uri_list = imgtable.get_image_uris_original(
        gid_list
    )  # A list of original image paths
    gps_list = imgtable.get_image_gps(gid_list)
    ggr_hierarchy = (
        []
    )  # A list of days, each a dictionary containing cars, each containing 3 aligned lists
    ...
    print(
        f"[extrapolate_ggr_gps] {len(missing_gid_list)}/{len(gid_list)} total images missing GPS data"
    )
    ...
    for i in range(len(gid_list)):
        ...
        if doctest_mode:
            print(
                "[extrapolate_ggr_gps] gathering image data [%d/%d]\n"
                % (i + 1, len(gid_list)),
                end="",
            )
        else:
            print(
                "[extrapolate_ggr_gps] gathering image data [%d/%d]\r"
                % (i + 1, len(gid_list)),
                end="",
            )
        ...
        for car in day.keys():
            ...
            for cam_idx in range(len(car_data[0])):
                ...
                for gid_idx in range(len(car_data[0][cam_idx])):
                    ...
                    image = cv2.cvtColor(
                        cv2.imread(uri_lookup[qr_image_gid]), cv2.COLOR_BGR2RGB
                    )
                    ...
                    if len(data) != 0:
                        ...
                        print(
                            "[extrapolate_ggr_gps] found QR code: {Car: %s; Camera: %s; Day: %d; Image: %s; URL: %s}"
                            % (
                                car,
                                chr(cam_idx + 65),
                                day_idx + 1,
                ...
                if car_data[1][cam_idx] == 0:
                    print(
                        "[extrapolate_ggr_gps] unable to locate QR code: {Car: %s; Camera: %s; Day: %d}"
                        % (
                            car,
                            chr(cam_idx + 65),
                            day_idx + 1,
        ...
        for car in ggr_hierarchy[day_idx].keys():
            ...
            for cam_idx in range(len(imgsets)):
                ...
                if qr_gids[cam_idx] != 0:
                    ...
                    imgtable.set_image_unixtimes(
                        gids, [t - timedeltas[cam_idx] for t in unixtimes]
                    )
                    print(
                        "[extrapolate_ggr_gps] synchronized unixtimes: {Car: %s; Camera: %s; Day: %d}"
                        % (
                            car,
                            chr(cam_idx + 65),
                            day_idx + 1,
                ...
                if cam_idx == gps_cam_idx:
                    ...
                    for i in range(len(gids_sorted)):
                        ...
                        if imgtable.get_image_gps(gids_sorted[i]) == (-1, -1):
                            ...
                            j = i
                            while j > 0 and imgtable.get_image_gps(gids_sorted[j]) == (
                                -1,
                                -1,
                            ):
                                j -= 1
                            ...
                            k = i
                            while (k < len(gids_sorted) - 1) and imgtable.get_image_gps(
                                gids_sorted[k]
                            ) == (-1, -1):
                                k += 1
                            ...
                            if abs(times_sorted[j] - times_sorted[i]) > abs(
                                times_sorted[k] - times_sorted[i]
                            ) or nearest_gps == (-1, -1):
                                nearest_gps = imgtable.get_image_gps(gids_sorted[k])
                            imgtable.set_image_gps([gids_sorted[i]], [nearest_gps])

                    print(
                        f"[extrapolate_ggr_gps] set camera {chr(cam_idx + 65)} as GPS reference for car {car} on day {day_idx + 1}"
                    )
                    break
    ...
    for i in range(len(gid_list)):
        ...
        if gps_list[i] == (-1, -1):
            ...
            c_lt_by_uuid[uuid_list[i]] = (None, None)
        else:
            ...
            c_lt_by_uuid[uuid_list[i]] = (c_name, lt_name)
            

    print(
        "[extrapolate_ggr_gps] set GPS data for %d / %d images with missing GPS entries"
        % (len(missing_gid_list) - len(skipped_gid_list), len(missing_gid_list))
    )
    if skipped_gid_list:
        print(
            "[extrapolate_ggr_gps] unable to extrapolate GPS data for %d images with the following gids:"
            % (len(skipped_gid_list))
        )
...
def match_point_to_poly(point, poly_prev, polys):
    """
    Find matching polygon in a list of polygons that a point lies within or touches. Checks most recent polygon first.

    Parameters:
        point (shapely Point object): point coordinates
        poly_prev (shapely Polygon object): matching polygon for previous point
        polys (list): list of Polygon objects to check

    Returns:
        poly_cur (shapely Polygon object): the polygon that the point lies within or touches, or None if there is no such polygon
    """

    poly_cur = None

    if poly_prev and point.within(poly_prev):
                poly_cur = poly_prev
    else:
        for p in polys:
...
def get_ggr_polygons(filepath="ggr_counties.json", c_or_lt=0, poly_obj=True, invert=False):
    """
    ...
    for county in in_dict["features"]:
        ...
        if county["geometry"]["type"] == "Polygon":
            coord_list = [
                (point[0], point[1]) for point in county["geometry"]["coordinates"][0]
            ]
            ...
            if poly_obj:
                poly_dict[name] = MultiPolygon(poly_dict[name])

    land_type = "land tenures" if c_or_lt else "counties"
    
    print(
        f"[ggr_funcs.get_ggr_polygons] Imported polygons for {len(poly_dict)} {land_type} from json..."
    )
    print(poly_obj)

    if invert:
        inv_poly_dict = {value: key for key, value in poly_dict.items()}
        return inv_poly_dict
...
def match_ggr_polygons(imgtable, c_path, lt_path):
    ...
    print(f"[ggr_funcs.match_ggr_polygons] Found counties for ({c_ct}/{len(gid_list)}) images")
    print(f"[ggr_funcs.match_ggr_polygons] Found land tenures for ({lt_ct}/{len(gid_list)}) images")
