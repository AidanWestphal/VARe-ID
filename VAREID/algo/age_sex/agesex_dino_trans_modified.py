...
from albumentations import Compose, Normalize, Resize, HorizontalFlip, ShiftScaleRotate, ColorJitter, CoarseDropout, OneOf, MotionBlur, GaussianBlur, ISONoise, GaussNoise
...
def main(args):
    ...
    lr_head = config.get('lr', 1e-4)
    lr_backbone = lr_head / 10 # 10x smaller learning rate for the pretrained backbone
    ...
    # CREATE ISOLATED BACKBONES FOR EACH TASK
    dinov3_age = copy.deepcopy(dinov3_base)
    dinov3_sex = copy.deepcopy(dinov3_base)

    # Freezing/Unfreezing Logic
    def prepare_backbone(backbone):
        for param in backbone.parameters():
            param.requires_grad = False
        # Unfreeze ONLY the last transformer block and layer norm
    ...
    print("Partial unfreezing initialized.")
    dinov3_age = prepare_backbone(dinov3_age)
    dinov3_sex = prepare_backbone(dinov3_sex)

    age_model = DINOProbe(dinov3_age, num_classes=6).to(device)
    sex_model = DINOProbe(dinov3_sex, num_classes=2).to(device)
    ...
    train_transforms = Compose([
        ...
        OneOf([GaussNoise(p=0.5), ISONoise(p=0.5)], p=0.5),
        OneOf([GaussianBlur(p=0.5), MotionBlur(p=0.5)], p=0.5),
    ...
    # Give the pretrained backbone 1e-5, give the random transformer pooler 1e-4
    age_param_groups = [
        {'params': age_model.backbone.blocks[-1].parameters(), 'lr': lr_backbone},
        {'params': age_model.backbone.norm.parameters(), 'lr': lr_backbone},
        {'params': age_model.pooler.parameters(), 'lr': lr_head},
        {'params': age_model.fc.parameters(), 'lr': lr_head}
    ]
    sex_param_groups = [
        {'params': sex_model.backbone.blocks[-1].parameters(), 'lr': lr_backbone},
        {'params': sex_model.backbone.norm.parameters(), 'lr': lr_backbone},
        {'params': sex_model.pooler.parameters(), 'lr': lr_head},
        {'params': sex_model.fc.parameters(), 'lr': lr_head}
    ]

    age_optimizer = torch.optim.AdamW(age_param_groups, weight_decay=0.05)
    sex_optimizer = torch.optim.AdamW(sex_param_groups, weight_decay=0.05)
    
    # AGE = LABEL SMOOTHING (Because age is a continuous spectrum)
    # SEX = STANDARD CROSS ENTROPY (Binary, distinct classes)
    criterion_age = nn.CrossEntropyLoss(label_smoothing=0.15)
    ...
    last_age_model_path = os.path.join(args.checkpoint_dir, f"last_patch_age_{run_name}.pth")
    last_sex_model_path = os.path.join(args.checkpoint_dir, f"last_patch_sex_{run_name}.pth")
    ...
    print("\n" + "="*40 + "\nTRAINING AGE PROBE\n" + "="*40)
    train_epochs(
        age_model, age_optimizer, age_train_loader, age_val_loader, criterion_age, 
    ...
    print("\n" + "="*40 + "\nTRAINING SEX PROBE\n" + "="*40)
    train_epochs(
        sex_model, sex_optimizer, sex_train_loader, sex_val_loader, criterion_sex, 
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Age and Sex DINO Patch Classifier Training')
