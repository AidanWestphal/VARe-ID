...
from albumentations import Compose, Normalize, Resize, RandomBrightnessContrast
...
def train_one_epoch_profiled(model, dataloader, optimizer, criterion_age, criterion_sex, device, verbose=True):
    ...
    if verbose:
        print(f"\n--- Epoch Profiling Summary ---")
        print(f"Total Data CPU Time: {total_data_time:.2f}s")
        print(f"Total Fwd GPU Time:  {total_forward_time:.2f}s")
        print(f"Total Bwd GPU Time:  {total_backward_time:.2f}s")
        print(f"Total Epoch Time:    {total_data_time + total_forward_time + total_backward_time:.2f}s")
...
def evaluate(model, dataloader, criterion_age, criterion_sex, device, verbose=True):
    model.eval()
    running_loss = 0.0
    age_correct, sex_correct, total = 0, 0, 0
    ...
    for imgs, targets in tqdm(dataloader, desc="Evaluating", disable=not verbose):
        ...
        total += imgs.size(0)
        age_correct += (age_preds == age_targets).sum().item()
        sex_correct += (sex_preds == sex_targets).sum().item()
        
    avg_loss = running_loss / len(dataloader)
    age_acc = age_correct / total
    sex_acc = sex_correct / total
    
    print(f"Val Loss: {avg_loss:.4f} | Age Acc: {age_acc:.4f} | Sex Acc: {sex_acc:.4f}")
    return avg_loss, age_acc, sex_acc
...
def main(args):
    ...
    # CLI Override for resume just in case you use the --resume flag
    if args.resume:
        resume = True
    ...
    print(f"Allocated {num_workers} workers based on SLURM resources.")
    ...
    print(f"Loaded and extracted data in {time.time() - start_extract:.2f}s\n")
    ...
    if resume:
        # Check if the last model exists in the specified checkpoint directory
        if os.path.exists(last_model_path):
            print(f"\nLoading checkpoint from {last_model_path}...")
            checkpoint = torch.load(last_model_path, map_location=device)
            model.load_state_dict(checkpoint['model_state_dict'])
            optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
            start_epoch = checkpoint['epoch'] + 1
            # Try to grab best val loss, fallback to inf if it wasn't tracked
            best_val_loss = checkpoint.get('val_loss', float('inf')) 
            print(f"Resumed successfully. Starting at epoch {start_epoch+1}.\n")
        else:
            print(f"\nResume flag set to True, but {last_model_path} does not exist. Starting from scratch.\n")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        val_loss, val_age_acc, val_sex_acc = evaluate(
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'val_sex_acc': val_sex_acc
            }, best_model_path)
        else:
            print(f"Validation loss did not improve from {best_val_loss:.4f}.")
            
        # Always save the 'last' state to allow for resumption
        torch.save({
            'epoch': epoch,
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
            'val_loss': best_val_loss # Keep track of best loss so it doesn't reset on resume
        }, last_model_path)
