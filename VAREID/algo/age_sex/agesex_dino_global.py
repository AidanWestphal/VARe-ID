...
# ==========================================
# MODEL ARCHITECTURE (GLOBAL CLS PROBE)
# ==========================================
...
class DINOGlobalProbe(nn.Module):
    ...
    def forward(self, x):
        with torch.no_grad():
            ...
            global_token = features['x_norm_clstoken']
        return self.fc(global_token)
...
def main(args):
    ...
    run_name = "global_" + config.get('run_name', 'global_cls')
    ...
    age_model = DINOGlobalProbe(dinov3, num_classes=6).to(device)
    sex_model = DINOGlobalProbe(dinov3, num_classes=2).to(device)

    age_optimizer = torch.optim.AdamW(age_model.fc.parameters(), lr=lr)
    sex_optimizer = torch.optim.AdamW(sex_model.fc.parameters(), lr=lr)
