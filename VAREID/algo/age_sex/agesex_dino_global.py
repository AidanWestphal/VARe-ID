...
class DINOProbe(nn.Module):
    ...
    def forward(self, x):
        with torch.no_grad():
            # forward_features returns a dict. We want the global class token.
...
def main(args):
    ...
    last_age_model_path = os.path.join(args.checkpoint_dir, f"last_age_{run_name}_cls.pth")
    last_sex_model_path = os.path.join(args.checkpoint_dir, f"last_sex_{run_name}_cls.pth")
