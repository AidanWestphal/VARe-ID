...
def main(args):
    ...
    command = f'python -u -m VAREID.algo.id_region.id_region_filter {config["idr_out_path"]} {config["idr_filtered_out_path"]} {flag}'

    # Do not reset the logfile, build off of ia_logs
    logger = setup_logging(config["idr_logs"], reset_logfile=False)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Driver script to run the id region filtering component of the pipeline. Filters out all annotations not suitable for identification."
    )
