...
import utils as ut
from constants import VALID_COLNAMES
from algo.preproc import get_exif 
...
class ImageTable:
    ...
    def __init__(self, dir, categories=[]):
        ...
        Example:
            ...
            /mnt/c/Users/Julian/Research/Pipeline/data/images /mnt/c/Users/Julian/Research/Pipeline/data/trash ["grevy's zebra"]
        """
    ...
    def add_image_columns(self, colnames, params_list):
        ...
        if not self.table['gid']:
            ...
            if dup_ct:
                print(f"[table.add_image_columns] Skipping {dup_ct} duplicate image(s)...")
        ...
        for col_idx in range(len(colnames)):
            ...
            if colname not in VALID_COLNAMES:
                print(f"[table.add_image_columns] Invalid column name ({colname}): skipping column...")
                continue
            elif colname in self.table.keys():
                print(f"[table.add_image_columns] Duplicate column name ({colname}): skipping column...")
                continue
    ...
    def add_image_rows(self, colnames, params_list):
        ...
        if sorted(colnames_test) != sorted(self.table.keys()):
            print("[add_image_rows] Provided columns are not identical to table columns: aborting operation...")
            return self.table['gid']
        ...
        for col_idx in range(len(colnames)):
            if colnames[col_idx] == 'uuid':
                ...
                if old_ct:
                    print(f"[add_image_rows] Failed to add {old_ct} existing images to image table")
                if dup_ct:
                    print(f"[add_image_rows] Failed to add {dup_ct} duplicate images to image table")
            
        # Automatically assign gids to new images
        num_images = len(self.table['gid'])
        self.table['gid'].extend(list(range(num_images + 1, num_images + len(params_list) + 1)))
        # Add data to table for each column provided
        for col_idx in range(len(colnames)):
            for row_idx in range(len(params_list)):
    ...
    def delete_images(self, gid_list, trash_images=True):
        ...
        Example:
            >>> from PIL import Image
            >>> import os
            >>> db_path = '/mnt/c/Users/Julian/Research/Pipeline/data/GGR2020_dataset/'
            ...
            >>> table = ImageTable("/mnt/c/Users/Julian/Research/Pipeline/data/GGR2020_dataset", ["grevy's zebra"])
            >>> table.add_image_data(['uuid', 'uri', 'original_name', 'ext'],
            ... [['0fdef8e8-cec0-b460-bac2-6ee3e39f0798', db_path + 'images/0fdef8e8-cec0-b460-bac2-6ee3e39f0798.JPG', 'DSCN5265', '.JPG'], 
            ... ['11ca4e9b-1de9-4ee5-8d90-615bf93077c8', db_path + 'images/11ca4e9b-1de9-4ee5-8d90-615bf93077c8.JPG', 'DSCN5266', '.JPG'],
            ... ['6830cd43-ca00-28ee-8843-fe64d789d7f7', db_path + 'images/6830cd43-ca00-28ee-8843-fe64d789d7f7.JPG', 'DSCN5267', '.JPG'],
            ... ['b54193f2-8507-8441-5bf3-cd9f0ed8a883', db_path + 'images/b54193f2-8507-8441-5bf3-cd9f0ed8a883.JPG', 'DSCN5268', '.JPG']])
            [1, 2, 3, 4]
            >>> table.delete_images([1, 3])
            [table.delete_images] deleting 2 images
            [utils.remove_file] Finished deleting path='/mnt/c/Users/Julian/Research/Pipeline/data/GGR2020_dataset/images/0fdef8e8-cec0-b460-bac2-6ee3e39f0798.JPG'
            [utils.remove_file] Finished deleting path='/mnt/c/Users/Julian/Research/Pipeline/data/GGR2020_dataset/images/6830cd43-ca00-28ee-8843-fe64d789d7f7.JPG'
            [1, 2]
            ...
            [utils.remove_file] Finished deleting path='/mnt/c/Users/Julian/Research/Pipeline/data/GGR2020_dataset/trash/DSCN5265'
            True
            >>> ut.remove_file(db_path + 'trash/DSCN5267')
            [utils.remove_file] Finished deleting path='/mnt/c/Users/Julian/Research/Pipeline/data/GGR2020_dataset/trash/DSCN5267'
            True
        """
        ...
        if trash_images:
            ...
            gpath_list2 = [
                join(self.trashdir, gname) for gname in gname_list
            ]
    ...
    def import_from_json(self, filepath):
        ...
        Example:
            ...
            [utils.remove_file] Finished deleting path='/mnt/c/Users/Julian/Research/Pipeline/data/GGR2020_dataset/test.json'
            True
        """
    ...
    def export_to_json(self, filepath):
        ...
        for cat_idx in range(len(self.categories)):
            out_dict["categories"].append({"id": cat_idx, "species": self.categories[cat_idx]})
    ...
    def get_imgset_gids(self, imgset_id_list):
        ...
        if type(imgset_id_list) == int:
            ...
            return [self.imgset_list[imgset_id - 1] for imgset_id in list(imgset_id_list)]
    ...
    def get(self, colnames, gid_list):
        ...
        if len(cols) == 1 and not ut.isiterable(gid_list):
            ...
            return [tuple(cols[col_idx][gid - 1] for col_idx in range(len(cols))) for gid in gid_list]
    ...
    def get_image_paths(self, gid_list):
        ...
        Example:
            ...
            '/mnt/c/Users/Julian/Research/Pipeline/data/images/DSCN5265.JPG'
            >>> gid_list = table.add_image_data(['uuid', 'uri'],
            ...
            ['/mnt/c/Users/Julian/Research/Pipeline/data/images/DSCN5265.JPG', '/mnt/c/Users/Julian/Research/Pipeline/data/images/DSCN5266.JPG', 
             '/mnt/c/Users/Julian/Research/Pipeline/data/images/DSCN5267.JPG', '/mnt/c/Users/Julian/Research/Pipeline/data/images/DSCN5268.JPG']
        """
