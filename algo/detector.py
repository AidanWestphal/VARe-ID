...
#
#
# def add_image_fname_column(df):
#     # Create the new 'image fname' column by appending '.jpg' to the 'image uuid' column
...
#             'annot species': 'neither' if annotation['annot species'] not in ['zebra_grevys', 'zebra_plains'] else annotation['annot species'],
...
#     parser.add_argument('exp_dir', type=str, help='The directory to export models and predictions to')
#     parser.add_argument('original_csv_path', type=str, help='The full path to the ground truth csv')
#     parser.add_argument('model_version', type=str, help='The yolo model version to use')
#     parser.add_argument('annots_csv_filename', type=str, help='The name of the output annotations csv file')
#     parser.add_argument('annots_filtered_csv_filename', type=str,
...
#     pred_df = load_annotations_from_csv(annot_csv_path)
#
#     # Add the 'image fname' column
#     pred_df = add_image_fname_column(pred_df)
#
#     # Save the updated DataFrame with the 'image fname' column back to CSV
#     save_annotations_to_csv({"annotations": pred_df.to_dict(orient='records')}, annot_csv_path)
#
#
...
#     save_annotations_to_json(predictions, filtered_annot_json_path)
#     #
...
#     save_annotations_to_csv({"annotations": pred_df.to_dict(orient='records')}, filtered_annot_csv_path)
...
def filtration(predicted_df, original_df, iou_thresh=0.50):
    ...
    filtered_annotations = [{
        ...
        'bbox pred score': annotation['bbox pred score'],
        'category id': annotation['category id'],
        'annot species': 'neither' if annotation['annot species'] not in ['zebra_grevys', 'zebra_plains'] else
        annotation['annot species'],
        'annot census': annotation["annot census"],
        'bbox gt': annotation['bbox gt'],
        'bbox pred': annotation['bbox pred'],
        'bbox x': annotation['bbox pred x'],
...
def join_ground_truth_predicted(ground_truth_path, predicted_path):
    # Load the CSV files
    ground_truth_df = pd.read_csv(ground_truth_path)
    predicted_df = pd.read_csv(predicted_path)

    # Remove ".jpg" extension from 'filename' in ground truth
    ground_truth_df["filename"] = ground_truth_df["filename"].str.replace(".jpg", "", regex=False)

    # Perform the join based on 'filename' and 'image uuid'
    joined_df = pd.merge(predicted_df, ground_truth_df, left_on="image uuid", right_on="filename", how="inner")

    return joined_df

# Example usage:
# joined_data = join_ground_truth_predicted("ground_truth.csv", "predicted.csv")
# print(joined_data)
...
if __name__ == '__main__':
    ...
    base_df = load_annotations_from_csv(annotations_csv_fullpath)
    print("Loading predicted annotations...")
    pred_df = load_annotations_from_csv(annot_csv_path)

    # join_dfcsv = join_ground_truth_predicted(base_df, pred_df)
    # join_annot_csv_path = os.path.join(annots, "joined_annotations.csv")
    # save_annotations_to_csv(join_dfcsv, join_annot_csv_path)
