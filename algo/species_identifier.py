...
def get_bioCLIP(url, target_dir):
    try:
        result = subprocess.run(['pip', 'install', f'git+{url}', '--target', target_dir], capture_output=True, check=True)
...
def postprerocess_dataframe(df) :
  ...
  df['species_true_simple'] = df['annot species'].apply(lambda x: simplify_species(x, category_map_true))

  category_map_pred = {"grevy's zebra": 0, 'plains zebra': 1, "neither": 2}
  df['species_pred_simple'] = df['species_prediction'].apply(lambda x: simplify_species(x, category_map_pred))
...
def plot_confusion_matrix(prediction_df, labels) :
    ...
    conf_matrix = confusion_matrix(true_labels, pred_labels, labels=true_labels.unique())
...
if __name__ == '__main__':
    ...
    parser.add_argument('si_dir', type=str, help='The directory to install bioCLIP within')
